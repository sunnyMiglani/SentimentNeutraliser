...
def removeTwitterData(tweet):
    ...
    # remove http(s?) links
    tweet = re.sub(r"https?\S+", "", tweet)  # remove any https
    # remove links that start with www.<name>\~\ .. etc
    tweet = re.sub(r"www[\.\w/~]+\S", "", tweet)
    
    # remove retweets, identified as they start with rt
...
def interaction():
    ...
    with open(inputName, newline="", encoding="ISO-8859-1") as csvfile:
        ...
        listOfTweets = [tweet[4] for tweet in listOfTweets]
        ...
        # outputName = input()
        outputName = "twitter_corpus_output"
